...
{
	auto fn = GET_EXTERNAL_PROC("SDL2", "SDL_SetRelativeMouseMode", int, (bool));
	if (fn) fn(enabled);
}

void CS2::EnableWindowPolling(bool enabled)
{
	auto fn = GET_EXTERNAL_PROC("SDL2", "SDL_SetWindowGrab", void, (void*, bool));
	if (fn) fn(InputSystem->GetSDLWindow(), enabled);
}

void CS2::SetMousePos(int x, int y)
{
	auto fn = GET_EXTERNAL_PROC("SDL2", "SDL_WarpMouseInWindow", void, (void*, int, int));
...
{
	auto CreateInterface = GET_EXTERNAL_PROC(moduleName, "CreateInterface", T*, (const char* versionString, int* returnCode));
